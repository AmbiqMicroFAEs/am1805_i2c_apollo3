...
//!
//!
//! This function performs a read to an AMx8x5 register over the I2C bus.
//!
//! @return
//
//*****************************************************************************
...
uint32_t status=AM_HAL_STATUS_SUCCESS;	
		...
		//
		// Send the register addres, don't stop, blocking transaction
		//
		buff[0] = ui8Register;
	  status = iom_i2c_write(RTC_SLAVE_ADDR, (uint32_t*)buff, 1, true, true);
		if(status != AM_HAL_STATUS_SUCCESS)return status;
	
		//
		// Read the register addres, finish the transaction, blocking
		//
		status = iom_i2c_read(RTC_SLAVE_ADDR, (uint32_t*)buff, 1, false, true);
		*pbuf = buff[0];
...
uint32_t status=AM_HAL_STATUS_SUCCESS;	
    ...
    buff[0] = ui8Register;
		...
		//
		// Write the register, blocking transaction
		//
		status = iom_i2c_write(RTC_SLAVE_ADDR, (uint32_t*)buff, 2, false, true);
...
uint32_t status=AM_HAL_STATUS_SUCCESS;	
		...
		//
		status = iom_i2c_read(RTC_SLAVE_ADDR, (uint32_t*)pui8Values, ui32NumBytes, false, true);
...
uint32_t status=AM_HAL_STATUS_SUCCESS;	
		...
		if(ui32NumBytes>128)
			{
			while(1){}; //Adjust the buffer size
			}
		...
		//
		buff[0] = ui8StartRegister;
		memcpy((void*)&buff[1],pui8Values,ui32NumBytes);
		
		//
		// Write the block of data, finish, blocking transaction
		//
		status = iom_i2c_write(RTC_SLAVE_ADDR, (uint32_t*)buff, ui32NumBytes+1, false, true);
